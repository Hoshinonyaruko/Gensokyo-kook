...
func NewProcessor(Token string, BaseUrl string, settings *config.Settings, wsclient []*wsclient.WebSocketClient, BotID uint64) *Processors {
	return &Processors{
		Token:    Token,
		BaseUrl:  BaseUrl,
		Settings: settings,
...
// 修改函数的返回类型为 *Processor
func NewProcessorV2(Token string, BaseUrl string, settings *config.Settings, BotID uint64) *Processors {
	return &Processors{
		Token:    Token,
		BaseUrl:  BaseUrl,
		Settings: settings,
...
// switch v := data.(type) {
// case *event.MessageKMarkdownEvent:
// 	realid = v.Author.ID
// }
...
// 		Content: "欢迎使用gensokyo-kook框架部署QQ机器人",
// 		MsgType: 0,
// 		MsgID:   "",
// 	}
...
func SendMessage(messageText string, data interface{}, messageType string, Token string, BaseUrl string) error {
	// 强制类型转换，获取Message结构
	var msg *event.MessageKMarkdownEvent
	switch v := data.(type) {
	case *event.MessageKMarkdownEvent:
		msg = (*event.MessageKMarkdownEvent)(v)
	default:
		return nil
	}
