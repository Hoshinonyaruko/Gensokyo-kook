...
func CombinedMiddleware(Token string, BaseUrl string) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if c.Request.URL.Path == "/send_group_msg" {
			handleSendGroupMessage(c, Token, BaseUrl)
			return
		}
		if c.Request.URL.Path == "/send_private_msg" {
			handleSendPrivateMessage(c, Token, BaseUrl)
			return
		}
...
func handleSendGroupMessage(c *gin.Context, Token string, BaseUrl string) {
	var retmsg string
	var req struct {
	...
	// 调用处理函数
	retmsg, err := handlers.HandleSendGroupMsg(client, Token, BaseUrl, message)
	if err != nil {
...
func handleSendPrivateMessage(c *gin.Context, Token string, BaseUrl string) {
	var retmsg string
	var req struct {
		GroupID    int64  `json:"group_id" form:"group_id"`
	...
	// 调用处理函数
	retmsg, err := handlers.HandleSendPrivateMsg(client, Token, BaseUrl, message)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func handleSendGuildChannelMessage(c *gin.Context, Token string, BaseUrl string) {
	...
	// 调用处理函数
	retmsg, err := handlers.HandleSendGuildChannelMsg(client, Token, BaseUrl, message)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
