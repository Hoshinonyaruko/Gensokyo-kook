...
type HandlerFunc func(client Client, Token string, BaseUrl string, messgae ActionMessage) (string, error)
...
func CallAPIFromDict(client Client, Token string, BaseUrl string, message ActionMessage) string {
	...
	jsonString, err := handler(client, Token, BaseUrl, message)
	if err != nil {
		// 处理错误
