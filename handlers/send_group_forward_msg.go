...
func HandleSendGroupForwardMsg(client callapi.Client, Token string, BaseUrl string, message callapi.ActionMessage) (string, error) {
	...
	for _, nodeInterface := range nodes {
		...
		if ok {
			...
			messageText, _ = parseMessageContent(callapi.ParamsContent{Message: content}, message, client, Token, BaseUrl)
		} else {
			// 处理直接包含的文本内容
		...
		HandleSendGroupMsg(client, Token, BaseUrl, newMessage)
		count++
