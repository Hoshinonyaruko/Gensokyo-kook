...
func NoMoreDoubleClick() error {
	toHighDPI()
	r := boxW(getConsoleWindows(), "请勿通过双击直接运行本程序, 这将导致一些非预料的后果.\n请在shell中运行./gensokyo-kook.exe\n点击确认将释出安全启动脚本，点击取消则关闭程序", "警告", 0x00000030|0x00000001)
	if r == 2 {
		return nil
	}
	r = boxW(0, "点击确认将覆盖gensokyo-kook.bat，点击取消则关闭程序", "警告", 0x00000030|0x00000001)
	if r == 2 {
		return nil
	}
	f, err := os.OpenFile("gensokyo-kook.bat", os.O_CREATE|os.O_RDWR, 0o666)
	if err != nil {
		return err
	}
	if err != nil {
		mylog.Printf("打开gensokyo-kook.bat失败: %v", err)
		return nil
	}
	_ = f.Truncate(0)
	...
	_, err = f.WriteString("%Created by gensokyo-kook. DO NOT EDIT ME!%\nstart cmd /K \"" + exPath + "\"")
	if err != nil {
		mylog.Printf("写入gensokyo-kook.bat失败: %v", err)
		return nil
	}
	f.Close()
	boxW(0, "安全启动脚本已生成，请双击gensokyo-kook.bat启动", "提示", 0x00000040|0x00000000)
	return nil
}
